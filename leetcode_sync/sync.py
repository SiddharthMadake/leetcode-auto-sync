...
from .paths import solution_relpath
...
def build_record(detail) -> SolutionRecord | None:
    if detail.status_display.strip().lower() != "accepted":
        LOGGER.info(
            "Skipping submission %s because status is %r",
    ...
    if not detail.code.strip():
        raise LeetCodeAPIError(
            f"Submission {detail.submission_id} is accepted "
    ...
    if spec is None:
        LOGGER.warning(
            "Unsupported LeetCode language %r on submission %s; skipping",
            detail.language,
            detail.submission_id,
        )
        return None
    ...
    if not problem_id.isdigit():
        raise LeetCodeAPIError(
            f"Submission {detail.submission_id} has no numeric problem ID"
        )

    if not detail.title_slug:
        raise LeetCodeAPIError(
            f"Submission {detail.submission_id} has no problem slug"
        )
    ...
    return SolutionRecord(
        ...
        leetcode_url=(
            f"https://leetcode.com/problems/{detail.title_slug}/"
        ),
...
def _keep_newest_by_problem_language(
...
def sync(settings) -> dict[str, object]:
    ...
    accepted = client.recent_accepted(
        settings.leetcode_username,
        settings.sync_lookback_limit,
    )

    LOGGER.info(
        "LeetCode returned %s recent accepted submissions",
        len(accepted),
    )
    ...
    for submission in accepted:
        try:
            detail = client.submission_detail(
            ...
            LOGGER.error(
                "LeetCode authentication failed while processing "
                "submission %s (%s).",
                submission.submission_id,
                submission.title,
            )
    ...
    for record in candidates:
        generated = write_solution(
            repo_root,
            record,
        )

        root_readme = update_root_readme(
            repo_root,
            settings.leetcode_username,
        )
        ...
        if not paths:
            LOGGER.info(
                "%s was already present after generation; "
                "skipping commit",
                record.title,
            )
            continue
